...
std::string DiscoveryString::construct(const std::string& service_name, const std::string& ip_address, unsigned short port)
{
    ...
    return service_name + ":" + ip_address + ":" + std::to_string(port) + ":" + std::to_string(timestamp_ms);
}

std::optional<DiscoveryInfo> DiscoveryString::parse(const std::string& response)
{
    // Expected format: "SERVICE_NAME:IP:PORT:TIMESTAMP" (e.g., "my_service:192.168.1.100:54321:1700000000000")
    ...
    // Find the third colon separator (between port and timestamp)
    size_t third_colon_pos = response.find(':', second_colon_pos + 1);
    if (third_colon_pos == std::string::npos)
    {
        EESTV_LOG_ERROR("Invalid discovery response format (missing third colon): " << response);
        return std::nullopt;
    }

    // Split into service name, IP, port, and timestamp parts
    ...
    std::string port_str     = response.substr(second_colon_pos + 1, third_colon_pos - second_colon_pos - 1);
    std::string timestamp_str = response.substr(third_colon_pos + 1);
    ...
    try
    {
        timestamp_ms = std::stoull(timestamp_str);
    }
    catch (const std::exception& e)
    {
        EESTV_LOG_ERROR("Invalid discovery response format (invalid timestamp '" << timestamp_str << "'): " << response);
        return std::nullopt;
    }
    ...
    return DiscoveryInfo {service_name, // Service name from the string
                          ip_str, port, endpoint, timestamp_ms};
}
